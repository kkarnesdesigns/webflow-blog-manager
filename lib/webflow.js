...
class WebflowAPI {
  ...
  // Upload image to a CMS item field
  // Webflow API v2 accepts image URLs or base64 data for Image fields
  async uploadImageToItem(collectionId, itemId, fieldSlug, imageData, isLive = false) {
    const endpoint = isLive
      ? `/collections/${collectionId}/items/${itemId}/live`
      : `/collections/${collectionId}/items/${itemId}`;

    // For Webflow CMS, images can be set via URL or file upload
    // Using the fieldData approach with image URL
    return this.request(endpoint, {
      method: 'PATCH',
      body: JSON.stringify({
        fieldData: {
